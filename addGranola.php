...
$checkQuery = "SELECT * FROM granolas WHERE granola_id = '$granolaName' AND account_id = $account_id";
$checkResult = $conn->query($checkQuery);

// checks if item with the specified granola_id and account_id already exists in the database for the given account.
// the item exists in granolas table:
if ($checkResult->num_rows > 0) {
    ...
    if (count($_POST) > 2) {
        ...
        $granola_unit_select_addToCart = $_POST['granola_unit_select_addToCart'];
        $granola_quantity_addToCart = $_POST['granola_quantity_addToCart'];

        $isStarred = ($isStarred === "true") ? 1 : 0;
        $is_selected = ($is_selected === "true") ? 1 : 0;

        $granola_quantity = $_POST['granola_quantity'];
        $granola_unit_select = $_POST['granola_unit_select'];

        if ($item_date !== "") {
            // Update item with provided date
            $updateQuery = "UPDATE granolas
                            SET quantity = '$quantity', note = '$note', item_date = STR_TO_DATE('$item_date', '%Y-%m-%d'), unit = '$unit', 
                                ...
                                , minQuantity = '$granola_quantity', unit_minQuantity = '$granola_unit_select'
                                , unit_addToCart = '$granola_unit_select_addToCart', addToCart = '$granola_quantity_addToCart'
                            WHERE granola_id = '$granolaName' AND account_id = $account_id";
    ...
    $insertQuery = "INSERT INTO granolas (granola_id, quantity, note, unit, is_starred, is_selected, account_id, minQuantity, unit_minQuantity)
                    VALUES ('$granolaName', 0, '', 'kg', 0, 0, '$account_id', 0, 'kg')";
