...
<head>
    ...
    <link rel="stylesheet" href="meat_fish.css">
    <style>
...
<body>
    ...
    <div class="search-wrapper">
        <label for="search" >Meat, chicken and fish Search:</label>
        <input type="search" id="search" data-search  placeholder="enter a Meat, chicken and fish item" oninput="searchItems()">
    </div>
    ...
    <div id="popup" class="popup">
        <div class="popup-content">
            ...
            <h3>Meat, chicken and fish List:</h3>
            ...
            <ul class="items-container-window">
                ...
                <li class="selectable-window" id="amnon fish-window" onclick="selectItem('amnon fish-window')">
                <img src="meat_fish/amnon fish.jpg" alt="amnon_fish">
                <p>Amnon fish</p>
                </li>

                <li class="selectable-window" id="beef-window" onclick="selectItem('beef-window')">
                <img src="meat_fish/beef.jpg" alt="beef">
                <p>beef</p>
                </li>

                <li class="selectable-window" id="chicken-window" onclick="selectItem('chicken-window')">
                <img src="meat_fish/chicken.png" alt="chicken">
                <p>chicken</p>
                </li>

                <li class="selectable-window" id="Entrecote-window" onclick="selectItem('Entrecote-window')">
                <img src="meat_fish/Entrecote.jpg" alt="Entrecote">
                <p>Entrecote</p>
                </li>

                <li class="selectable-window" id="frozen amnon fish-window" onclick="selectItem('frozen amnon fish-window')">
                <img src="meat_fish/frozen amnon fish.jpg" alt="frozen amnon fish">
                <p>frozen amnon fish</p>
                </li>

                <li class="selectable-window" id="hamburger-window" onclick="selectItem('hamburger-window')">
                <img src="meat_fish/hamburger.jpg" alt="hamburger">
                <p>hamburger</p>
                </li>

                <li class="selectable-window" id="kebab-window" onclick="selectItem('kebab-window')">
                <img src="meat_fish/kebab.png" alt="kebab">
                <p>kebab</p>
                </li>

                <li class="selectable-window" id="pullet chicken-window" onclick="selectItem('pullet chicken-window')">
                <img src="meat_fish/pullet chicken.jpg" alt="pullet chicken">
                <p>pullet chicken</p>
                </li>

                <li class="selectable-window" id="sausage-window" onclick="selectItem('sausage-window')">
                <img src="meat_fish/sausage.jpg" alt="sausage">
                <p>sausage</p>
                </li>
...
window.addEventListener('scroll', () => {
    ...
    var meatIds = ['amnon fish', 'beef', 'chicken', 'Entrecote', 'frozen amnon fish', 'hamburger', 'kebab', 'pullet chicken', 'sausage'];
...
function openPopupMinQuantity(meatName) {
    var popupElement = document.getElementById("meat-popup" + meatName);
    var buttonElement = document.getElementById('topLeftButton' + meatName);

    if (popupElement && buttonElement) {
        popupElement.classList.add('meat-popup');

        popupElement.style.display = "block";
        buttonElement.style.display = 'none';
    } else {
        console.error("Popup or button element not found for", meatName);
    }
}

function openPopupAddToCart(meatName) {
    var popupElement = document.getElementById("meat-popup-addToCart" + meatName);
    var buttonElement = document.getElementById('topLeftButton-AddToCart' + meatName);

    if (popupElement && buttonElement) {
        popupElement.classList.add('meat-popup-addToCart');
...
function closePopupMinQuantity(meatName) {
    var popupElement = document.getElementById("meat-popup" + meatName);
    var buttonElement = document.getElementById('topLeftButton' + meatName);

    if (popupElement && buttonElement) {
        // Remove the specific class when closing the popup
        popupElement.classList.remove('meat-popup');
...
function closePopupAddToCart(meatName) {
    var popupElement = document.getElementById("meat-popup-addToCart" + meatName);
    var buttonElement = document.getElementById('topLeftButton-AddToCart' + meatName);

    if (popupElement && buttonElement) {
        popupElement.classList.remove('meat-popup-addToCart');
...
function addItemToMainContainer() {
    ...
    if (selectedItems.length > 0) {
        ...
        selectedItems.forEach(function(selectedItem) {
            // Get the meat name (assuming it's in the alt attribute of the image)
            var meatName = selectedItem.querySelector('img').alt;
            ...
            if (!existingItem) {
                ...
                // Make an AJAX request to addMeat.php to add default information to the item in the database
                ...
                xhr.onreadystatechange = function () {
                    if (xhr.readyState == 4 && xhr.status == 200) {
                        ...
                        var quantityInput = document.getElementById('quantity' + meatName);
                        quantityInput.value = curQuantity;

                        var curNote = jsonResponse.existingNote;
                        var NoteInput = document.getElementById('noteText' + meatName);
                        NoteInput.value = curNote;

                        var curUnit  = jsonResponse.existingUnit;
                        var UnitInput = document.getElementById('unit' + meatName);
                        ...
                        var UnitMinQuantityInput = document.getElementById('meat-unit' + meatName);
                        UnitMinQuantityInput.value = curUnitMinQuantity;

                        var curAddToCart = jsonResponse.existingAddToCart;
                        var AddToCartInput = document.getElementById('meat-quantity-addToCart' + meatName);
                        AddToCartInput.value = curAddToCart;

                        var curUnitAddToCart = jsonResponse.existingUnitAddToCart;
                        var UnitAddToCartInput = document.getElementById('meat-unit-addToCart' + meatName);
                        UnitAddToCartInput.value = curUnitAddToCart;
                        
                        var curDate = jsonResponse.existingDate;
                        var DateInput = document.getElementById('Date' + meatName);
               ...
               var saveNoteButton = newItem.querySelector('#saveNoteButton' + meatName);
                saveNoteButton.addEventListener('click', function () {
                    ...
                    alert("Item '" + meatName + "' is already in the main container.");
                }
            });
...
function handleItemChange(event) {
    ...
    if (item) {
        ...
        const meatId = item.id.replace('main-', '');
        ...
        const starIcon = document.querySelector('[id^="starIcon' + meatId + '"]');
        ...
        const meat_quantity = item.querySelector('.meat-quantity').value;
        const meat_unit_select = item.querySelector('.meat-unit-select').value;

        const meat_quantity_addToCart = item.querySelector('.meat-quantity-addToCart').value;
        const meat_unit_select_addToCart = item.querySelector('.meat-unit-select-addToCart').value;

        // Use AJAX or another method to send the data to addMeat.php
        ...
        const data = `meatName=${meatId}&meat_quantity=${meat_quantity}&meat_unit_select=${meat_unit_select}&account_id=${account_id}&quantity=${quantity}&note=${note}&unit=${unit}&isStarred=${isStarred}&is_selected=${isSelected}&date=${date}&meat_quantity_addToCart=${meat_quantity_addToCart}&meat_unit_select_addToCart=${meat_unit_select_addToCart}`;
        xhr.send(data);

        // Log the changes to the console
        console.log(`Item changed: ${meatId}, Quantity: ${quantity}, Starred: ${isStarred}, Selected: ${isSelected}, Date: ${date}`);

        const quantityContainer = document.getElementById(`quantityContainer${meatId}`);
        /*
        console.log(meatId);
        if (meat_quantity > quantity ){
            console.log("error");
            document.getElementById(`quantityContainer${meatId}`).classList.add('warning');
        } else {
            document.getElementById(`quantityContainer${meatId}`).classList.remove('warning');
        }
        */


        // Get the unit of the minimum quantity from the database
        const minQuantityUnit = item.querySelector('.meat-unit-select').value;


       
        if (parseFloat(meat_quantity) > parseFloat(quantity) && minQuantityUnit===unit) {
...
function addItem() {
    ...
    const meatName = document.getElementById('meatName').value;
    const quantity = item.querySelector('.quantity-input').value;
    const note = item.querySelector('.note-text').value;
    const unit = item.querySelector('.unit-select').value;
    ...
    // Encode data and send the request
    const data = `&meatName=${meatName}&meat_quantity=${meat_quantity}&meat_unit_select=${meat_unit_select}&account_id=${account_id}&date=${date}&quantity=${quantity}&note=${note}&unit=${unit}&isStarred=${isStarred}&is_selected=${isSelected}`;
    xhr.send(data);
}
...
function fetchStarredItems() {
    ...
    xhrFetchStarredItems.open("POST", "fetchStarredItemsMeat.php", true);
...
function addItemToMainContainerStar(meatId) {
    console.log(meatId);
    ...
    if (!existingItem) {
        ...
        newItem.innerHTML = `
                ...
                <div>
                    ...
                    <div class="favorite">
                        <i id="starIcon${meatName}" class="fas fa-star"></i> 
                    </div>
                `;
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                var curMinQuantity= jsonResponse.existingMinQuantity;
                var MinQuantityInput = document.getElementById('meat-quantity' + curFruit);
                MinQuantityInput.value = curMinQuantity;

                var curUnitMinQuantity = jsonResponse.existingUnitMinQuantity;
                var UnitMinQuantityInput = document.getElementById('meat-unit' + curFruit);
                UnitMinQuantityInput.value = curUnitMinQuantity;

                var curAddToCart = jsonResponse.existingAddToCart;
                var AddToCartInput = document.getElementById('meat-quantity-addToCart' + curFruit);
                AddToCartInput.value = curAddToCart;

                var curUnitAddToCart = jsonResponse.existingUnitAddToCart;
                var UnitAddToCartInput = document.getElementById('meat-unit-addToCart' + curFruit);
                UnitAddToCartInput.value = curUnitAddToCart;
